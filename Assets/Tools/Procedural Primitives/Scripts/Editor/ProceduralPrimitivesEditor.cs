...
{
    public class ProceduralPrimitivesEditor
    {
        ...
        static void CreateInstance(MenuCommand menuCommand)
        {
            ...
            if (context != null)
            {
                go.GetComponent<PPInstance>().ApplySource(context.GetComponent<PPBase>());
                go.transform.parent = context.transform.parent;
                go.transform.SetSiblingIndex(context.transform.GetSiblingIndex() + 1);
                go.transform.position = context.transform.position;
                go.transform.rotation = context.transform.rotation;
